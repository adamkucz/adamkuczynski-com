...
- `lm()` returns lists nested in a list of size 12
--
...
One of R's greatest features is that ability to extend the functionality of base R with **packages** (bundles of code that others have written to perform a set of tasks)
--


*Anybody* can write a package, which means that there are a lot of packages available - 17,705 to be exact - for you to use (for free!). If you need to do something base R can't, chances are there's a package for it
--
...
- `arglist` is a common separated list of the arguments that your function will take
...
R will search as many parent environments as it needs to find a object, but it will *never* search downwards
...
```{r}
pasteStrings <- function(...){
  arglist <- unlist(list(...))
  string <- paste0(arglist, collapse = ", ")
  return(string)
}
pasteStrings("a", "b", "c")
pasteStrings("Apples", "Bananas", "Carrots", "Dates")
```
---
# `...`
...
.small[
The `magrittr` package in R introduced the **pipe** operator<sup>1</sup> (`%>%`) to make nested function calls more intuitive to write and easier to read
...
```
- `%<>%` 'assignment' pipe: the result of the pipe is assigned to the lhs  object (equivalent to `lhs <- lhs %>% rhs`). You are generally discouraged from using this pipe.
<div>
...
- `+=` (add the rhs to the lhs: `x <- x + rhs`)
- `-=` (subtracts the rhs from the lhs: `x <- x - rhs`)
- `++` (adds one to a variable: `x <- x + 1`)
- `--` (subtracts one from a variable: `x <- x - 1`)
...
Methods are denoted by `.{class_name}` after the generic function name. For example, let's take a look at the `summary` generic function, which has `r length(methods(summary))` methods:
...
- `...` is a list of arguments to vectorize over
...
Let's simulate 10,000 samples of *n* = 300 from a uniform distribution (equal probability of all values, 0 to 10) and plot the means. **What do you expect the distribution to look like?**

```{r, fig.height=3, fig.width=4.5, fig.align='center', dev='svg'}
hist(
  replicate(10000, mean(runif(300, 0, 10))), #<<
  main = "", xlab = "Mean", breaks = 25
)
```
]
---
...
- `FUN`: the function to be used to carry out the sweep (default is to substract_)
...
.pull-right[
Mean miles per gallon by automatic (0) or manual (1) transmission *and* engine shape (0 = V-shaped, 1 = straight)
```{r}
