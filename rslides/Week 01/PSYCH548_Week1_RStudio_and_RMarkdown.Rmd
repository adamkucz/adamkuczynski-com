...
---
# Required Software
- R ([Version 4.0+](https://cloud.r-project.org/))
...
- Slack: [https://psych548intro-e273612.slack.com](https://psych548intro-e273612.slack.com)
...
2. R basics (data structures, objects, classes, methods) and style guide
3. Control structures (`if`/`else`, `switch`, `for`, `while`, `repeat`, `break`, `next`) and vectorization
...
---
# Slide Formatting
`Code` represents R code you type into the editor or console or keystrokes used to perform actions, for example: "you can use the `mean()` function to find the average of a vector of numbers. Sometimes `median()` is a better summary, though."
...
- R is a programming language, so you can do almost anything you want with it (you don't have to reply on whatever is implemented in the software you use)
...
******
In the **console**, type `123 + 456 + 789` and hit <kbd>Enter</kbd>.
```{r}
123 + 456 + 789
```
...
If we didn't have a good guess as to what `sqrt()` will do, we can type `?sqrt` in the console (also `help(sqrt)` will work)
...
```
---
You can create a copy of the object by assigning it a new name. This does *not* clone the object (i.e., changing
...
Inside RMarkdown, lines of R code are called **chunks**. Code is placed between sets of three backticks and `{r}`. This chunk of code:
...
You can also reference an object than you created in a chunk:
```{r}
age <- 30
```

```{r, echo=FALSE, comment=NA}
cat("This year is he `r age` years old.")
```
